...
The API owns endpoint request/response shapes. `packages/shared` owns domain objects that are shared across packages. The recipe-builder domain is intentionally shallow for now and will be modeled later.
...
- `apps/api` exports a type-only RPC contract from `@recipes/api/rpc`.
- `apps/web` imports that contract with type-only imports and builds its client in `apps/web/src/api.ts`.
- API endpoint response shapes stay in `apps/api`; domain objects stay in `packages/shared`.
- TanStack Router route context carries `queryClient`.
- TanStack Router file routing owns route discovery; `apps/web/src/routeTree.gen.ts` is generated and committed.
